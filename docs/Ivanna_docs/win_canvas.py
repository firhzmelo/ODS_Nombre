...
def fetch_data():
    ...
    try:
        url = (
            ...
            "?latitude=1.2914&longitude=103.8639"
        ...
        temperaturas = data["hourly"]["precipitation"]
...
def create_line_chart(horas, temps):
    ...
    ax.plot(horas, temps, linestyle=":", marker="x", markersize=5,color="orange")
    ax.set_title("Precipitación en el circuito de Marina Bay(línea)")
    ax.set_xlabel("Hora")
    ax.set_ylabel("Milimetros")
    ax.tick_params(axis="x", rotation=110)
    fig.tight_layout()
    return fig
...
def create_bar_chart(horas, temps):
    ...
    fig, ax = plt.subplots(figsize=(6, 3))
    ax.bar(horas, temps)
    ax.set_title("Precipitación en el circuito de Marina Bay (barras)")
    ax.set_xlabel("Hora")
    ax.set_ylabel("Milimetros")
    ax.tick_params(axis="x", rotation=110)
    fig.tight_layout()
