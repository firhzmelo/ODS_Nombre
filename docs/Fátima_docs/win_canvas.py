...
def fetch_data():
    ...
    try:
        url = (
            ...
            "?latitude=22.59&longitude=45.46"
            "&daily=sunshine_duration&past_days=1"
            "&timezone=auto"
        )
        ...
        horas = data["daily"]["time"]
        temperaturas = data["daily"]["sunshine_duration"]
...
def create_line_chart(horas, temps):
    ...
    ax.plot(horas, temps, linestyle="--", marker="s", markersize=6, markeredgecolor="black", color="orange")
    ax.set_title("Sunshine Duration in Houston (line)")
    ax.set_xlabel("Date")
...
def create_bar_chart(horas, temps):
    """Gráfica de barras."""
    fig, ax = plt.subplots(figsize=(6, 3))
    colores = cm.viridis(np.linspace(0, 1, len(horas)))
    ax.bar(horas, temps, color=colores)
    ax.set_title("Sunshine Duration in Houston (bar)")
    ax.set_xlabel("Date")
    ax.set_ylabel("Seconds (s)")
    ax.tick_params(axis="x", rotation=45)
    fig.tight_layout()
